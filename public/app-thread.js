...
`;
    ...
    function _renderActionBar(area, postEl, rtCount, lkCount) {
        ...
        likeBtn.addEventListener('click', function () {
            ...
            likeBtn.querySelector('i').className = isOn ? 'fas fa-heart' : 'far fa-heart';
        ...
        bmBtn.addEventListener('click', function () {
            if (_isGuest()) { _openAuth(); return; }
            var isOn = bmBtn.classList.toggle('bookmarked');
            bmBtn.querySelector('i').className = isOn ? 'fas fa-bookmark' : 'far fa-bookmark';
    ...
    function _openShareSheet(text, postId) {
        ...
        if (postId && _fbOk()) {
            try { window.fbDb.collection('posts').doc(postId).update({ shareCount: _inc(1) }).catch(function(){}); } catch(e){}
        }
    ...
    var _SVG = {
        'far fa-comment':   '<svg viewBox="0 0 24 24" width="20" height="20" fill="none" stroke="currentColor" stroke-width="1.8" stroke-linecap="round" stroke-linejoin="round"><path d="M21 15a2 2 0 0 1-2 2H7l-4 4V5a2 2 0 0 1 2-2h14a2 2 0 0 1 2 2z"/></svg>',
        'fas fa-retweet':   '<svg viewBox="0 0 24 24" width="20" height="20" fill="none" stroke="currentColor" stroke-width="1.8" stroke-linecap="round" stroke-linejoin="round"><path d="M17 1l4 4-4 4"/><path d="M3 11V9a4 4 0 0 1 4-4h14"/><path d="M7 23l-4-4 4-4"/><path d="M21 13v2a4 4 0 0 1-4 4H3"/></svg>',
        'far fa-heart':     '<svg viewBox="0 0 24 24" width="20" height="20" fill="none" stroke="currentColor" stroke-width="1.8" stroke-linecap="round" stroke-linejoin="round"><path d="M20.84 4.61a5.5 5.5 0 0 0-7.78 0L12 5.67l-1.06-1.06a5.5 5.5 0 0 0-7.78 7.78l1.06 1.06L12 21.23l7.78-7.78 1.06-1.06a5.5 5.5 0 0 0 0-7.78z"/></svg>',
        'fas fa-share-alt': '<svg viewBox="0 0 24 24" width="20" height="20" fill="none" stroke="currentColor" stroke-width="1.8" stroke-linecap="round" stroke-linejoin="round"><circle cx="18" cy="5" r="3"/><circle cx="6" cy="12" r="3"/><circle cx="18" cy="19" r="3"/><line x1="8.59" y1="13.51" x2="15.42" y2="17.49"/><line x1="15.41" y1="6.51" x2="8.59" y2="10.49"/></svg>',
        'far fa-bookmark':  '<svg viewBox="0 0 24 24" width="20" height="20" fill="none" stroke="currentColor" stroke-width="1.8" stroke-linecap="round" stroke-linejoin="round"><path d="M19 21l-7-5-7 5V5a2 2 0 0 1 2-2h10a2 2 0 0 1 2 2z"/></svg>',
        'fas fa-chart-bar': '<svg viewBox="0 0 24 24" width="20" height="20" fill="currentColor"><rect x="1" y="12" width="4" height="10" rx="1"/><rect x="9" y="7" width="4" height="15" rx="1"/><rect x="17" y="2" width="4" height="20" rx="1"/></svg>',
        'fas fa-quote-right':'<svg viewBox="0 0 24 24" width="20" height="20" fill="none" stroke="currentColor" stroke-width="1.8" stroke-linecap="round" stroke-linejoin="round"><path d="M21 15a2 2 0 0 1-2 2H7l-4 4V5a2 2 0 0 1 2-2h14a2 2 0 0 1 2 2z"/></svg>',
        'fas fa-pen':       '<svg viewBox="0 0 24 24" width="20" height="20" fill="none" stroke="currentColor" stroke-width="1.8" stroke-linecap="round" stroke-linejoin="round"><path d="M12 20h9"/><path d="M16.5 3.5a2.121 2.121 0 0 1 3 3L7 19l-4 1 1-4 12.5-12.5z"/></svg>'
    };

    function _makeActBtn(iconClass, label) {
        var btn = document.createElement('button');
        btn.className = 'vf-th-act-btn';
        var iconHTML = _SVG[iconClass] || ('<svg viewBox="0 0 24 24" width="20" height="20" fill="none" stroke="currentColor" stroke-width="1.8"><circle cx="12" cy="12" r="10"/></svg>');
        btn.innerHTML =
        ...
        if (isRetweeted) {
            popup.innerHTML =
                '<button class="vf-rt-popup-item undo" data-rt-action="repost">' +
                    '<svg viewBox="0 0 24 24" width="18" height="18" fill="none" stroke="currentColor" stroke-width="1.8" stroke-linecap="round" stroke-linejoin="round"><path d="M17 1l4 4-4 4"/><path d="M3 11V9a4 4 0 0 1 4-4h14"/><path d="M7 23l-4-4 4-4"/><path d="M21 13v2a4 4 0 0 1-4 4H3"/></svg> Undo repost' +
                '</button>' +
                '<button class="vf-rt-popup-item" data-rt-action="quote">' +
                    '<svg viewBox="0 0 24 24" width="18" height="18" fill="none" stroke="currentColor" stroke-width="1.8" stroke-linecap="round" stroke-linejoin="round"><path d="M12 20h9"/><path d="M16.5 3.5a2.121 2.121 0 0 1 3 3L7 19l-4 1 1-4 12.5-12.5z"/></svg> Quote' +
                '</button>';
        } else {
            popup.innerHTML =
                '<button class="vf-rt-popup-item" data-rt-action="repost">' +
                    '<svg viewBox="0 0 24 24" width="18" height="18" fill="none" stroke="currentColor" stroke-width="1.8" stroke-linecap="round" stroke-linejoin="round"><path d="M17 1l4 4-4 4"/><path d="M3 11V9a4 4 0 0 1 4-4h14"/><path d="M7 23l-4-4 4-4"/><path d="M21 13v2a4 4 0 0 1-4 4H3"/></svg> Repost' +
                '</button>' +
                '<button class="vf-rt-popup-item" data-rt-action="quote">' +
                    '<svg viewBox="0 0 24 24" width="18" height="18" fill="none" stroke="currentColor" stroke-width="1.8" stroke-linecap="round" stroke-linejoin="round"><path d="M12 20h9"/><path d="M16.5 3.5a2.121 2.121 0 0 1 3 3L7 19l-4 1 1-4 12.5-12.5z"/></svg> Quote' +
                '</button>';
    ...
    function _doRepost(postEl) {
        ...
        /* Award mining */
        if (typeof window._awardImpactMining === 'function') {
            window._awardImpactMining('RETWEET_POST', postId || ('rt-' + Date.now()));
        }

        /* Inject repost card into feed */
        _injectRetweetCard({
        ...
        if (_fbOk()) {
            ...
            try {
                window.fbDb.collection('posts').add(doc).catch(function () {});
    ...
    function _injectRetweetCard(opts) {
        ...
        card.dataset.postId = 'rt-' + Date.now();
    ...
    function _doSubmitQuote() {
        ...
        /* Update Firestore retweet/quote count on the original post */
        if (postId && _fbOk()) {
            try {
                window.fbDb.collection('posts').doc(postId).update({
                    retweetCount: _inc(1),
        ...
        /* Mining */
        if (typeof window._awardImpactMining === 'function') {
            window._awardImpactMining('QUOTE_POST', postId || ('qt-' + Date.now()));
        }

        /* Inject quote card into feed */
        _injectQuoteCard({
            quoteText:   text,
    ...
    function _injectQuoteCard(opts) {
        var card = document.createElement('div');
        card.className = 'impact-story';
        card.dataset.postId = 'qt-' + Date.now();
    ...
    function _subscribeCounts() {
        ...
        try {
            _unsubCounts = window.fbDb.collection('posts').doc(_activePostId)
                .onSnapshot(function (doc) {
                    ...
                    if (bar) {
                        ...
                        if (rtBtn) rtBtn.textContent = rtCount || '';
                        if (lkBtn) lkBtn.textContent = lkCount || '';
                    }

                    /* Mirror back to source feed card */
                    if (_activePostEl) {
                        ...
                        if (srcRt) srcRt.textContent = String(rtCount);
                        if (srcLk) srcLk.textContent = String(lkCount);
                        if (srcCm) srcCm.textContent = String(cmCount);
                    }
    ...
    function _loadComments() {
        ...
        try {
            _unsubComments = window.fbDb.collection('posts').doc(_activePostId)
                .collection('comments')
        ...
        item.innerHTML =
            ...
            '<div class="vf-th-comment-right">' +
                ...
                '<div class="vf-th-comment-actions">' +
                    '<button class="vf-th-cmt-act vf-cmt-reply" title="Reply"><svg viewBox="0 0 24 24" width="18" height="18" fill="none" stroke="currentColor" stroke-width="1.8" stroke-linecap="round" stroke-linejoin="round"><path d="M21 15a2 2 0 0 1-2 2H7l-4 4V5a2 2 0 0 1 2-2h14a2 2 0 0 1 2 2z"/></svg> Reply</button>' +
    ...
    function _uploadToCloudinary(item) {
        return new Promise(function (resolve, reject) {
            ...
            var uploadPreset = _cld.preset || window.CLOUDINARY_UPLOAD_PRESET || window.cloudinaryUploadPreset || 'Empyrean_preset';
